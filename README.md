...
- **AI 推理窗格**（右侧）— 三栏布局的第三栏，承载全部「思考」内容：思维链（CoT）流式卡片、
  ...
  生成或结果分析失败时卡片上有一键**重试**（原样重放同一次请求，不重新改写问题），
  结果分析的重试是「重试分析」（只重新解读那段输出，不会重跑命令）；`Esc` 取消过的生成也能重试
...
- **本地安全引擎** — 基于 Go AST（`mvdan.cc/sh/v3`）而非正则：递归强制删除根目录、
  ...
  批量删除、**提权（`sudo`/`doas`/`su`）** 等需二次确认。**判定完全在本地毫秒级完成，不依赖 LLM**
- **提权命令不会被自动执行** — `sudo` 之类几乎一定会停下来等密码，而自动执行答不了这个问答：
  它会被归入黄区走影子推演，绝不静默下发；真下发后终端停在密码提示时，卡片会显示
  `🟡 等待输入`（不是转圈），并且那一段输出**留在捕获里继续等**——你输完密码、命令真正跑完，
  才会把真实输出交给模型分析。中途放弃则把这一段落标成「停在输入提示，未确认结果」，
  不会把密码提示当成执行成功的证据
...
- **黄区** — 右侧展开影子推演面板，列出命中的规则，点「确认执行」才下发（授权令牌一次性、2 分钟过期、
  绑定会话与命令原文）。提权命令（`sudo`/`doas`/`su`/`runuser`/`pkexec`）一律归在这一级：
  它们几乎必然要交互输入密码，自动执行只会把终端卡在提示符上；
- **红区** — 拒绝下发，输入框红框、回车被禁用。

> 卡片上的执行状态区分「⚡ 已自动执行（绿区）」与「🟢 已确认执行」（你在影子推演里点的
> 「确认执行」），两者不会混为一谈。
